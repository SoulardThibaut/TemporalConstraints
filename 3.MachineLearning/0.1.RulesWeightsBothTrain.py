    ...
    with open(f"./{path}{files_test}_{rules_to_use}_{id}", "w", encoding="UTF-8") as f:
        try: 
            while(True):
                ...
                entity.remove_triple(true_triple)


                for j, triple in enumerate([true_triple, false_triple]):
                    decision_per_rule_naive = []
                    decision_per_rule_remove = []
                    # 0 -> Followed
                    # 1 -> Refuted
                    ...
                    if len({triple.relation, (triple.relation, triple.value)}.intersection(rules_about)) == 0:
                        decision_per_rule_naive = [2]*len(rules)
                        decision_per_rule_remove = [2]*len(rules)
                    else:
                        if not triple.head in entities:
                            decision_per_rule_naive = [4]*len(rules)
                            decision_per_rule_remove = [4]*len(rules)
                        else:
                            ...
                            for rule in rules:
                                component_used = {triple.relation, (triple.relation, triple.value)}.intersection({rule.get_a(), rule.get_b()})
                                if len(component_used) == 0:
                                    decision_per_rule_naive.append(2)
                                    decision_per_rule_remove.append(2)
                                else:
                                    if tp.TimeSequence(tp.ordered_time_sequence_first_start_with_rxv(entity, rule.get_a())).multi_valuation_temporal:
                                        decision_per_rule_naive.append(3)
                                    else:
                                            ...
                                            if (len(ts_a.intervals) != 0) and (len(ts_b.intervals) != 0):
                                                ...
                                                if (rule.get_a() == tsr.name_relation_A):
                                                    if rule.get_r() in tsr.A_o_B:
                                                        if len(decision_per_rule_naive) == len(decision_per_rule_remove):
                                                            decision_per_rule_naive.append(0)
                                                        decision_per_rule_remove.append(0)
                                                    else:
                                                        if len(decision_per_rule_naive) == len(decision_per_rule_remove):
                                                            decision_per_rule_naive.append(1)
                                                        decision_per_rule_remove.append(1)
                                                else:
                                                ...
                                                if len(decision_per_rule_naive) == len(decision_per_rule_remove):
                                                    decision_per_rule_naive.append(4)
                                                decision_per_rule_remove.append(4)
                                        else:
